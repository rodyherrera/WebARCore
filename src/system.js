class SharedMemory{
    constructor(wasm, type, size){
        ...
        if(this.TypedArray === null){
            throw new Error('Unknown Data Type. Try "HEAP8"');
        }
...
class WebARCore{
    static async Initialize(width, height, fov = 45){
        const wasm = {};
        wasm.ready = WebARCoreWASM().then((module) => wasm.module = module);
        await wasm.ready;
        return new WebARCore(wasm, width, height, fov);
    }
    ...
    getCameraIntrinsics(width, height, fov = 45, near = 0.001, far = 1000){
        ...
        */
        ...
        let horizontalFovDegrees;
        let verticalFovDegrees;

        if(width > height){
            horizontalFovDegrees = fov * aspectRatio;
            verticalFovDegrees = fov;
        }else{
            horizontalFovDegrees = fov;
            verticalFovDegrees = fov * aspectRatio;
        }

        const degreesToRadians = (deg) => deg * 0.01745329251994329576;
        const principalPointX = width * 0.5;
        const principalPointY = height * 0.5;
        const focalLengthX = (width * 0.5) / Math.tan(degreesToRadians(horizontalFovDegrees * 0.5));
        const focalLengthY = (height * 0.5) / Math.tan(degreesToRadians(verticalFovDegrees * 0.5));
